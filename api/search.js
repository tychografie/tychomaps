...
module.exports = async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).json({ error: "tycho wants your body" });
  ...
  if (query.length > 128 || query.length === 0) {
    res.status(400).json({ error: "not good" });
    return;
  }
  try {
    // OpenAI API Call
    ...
    if (aiResponse.data.choices && aiResponse.data.choices.length > 0) {
      ...
      if (mapsResponse.data && mapsResponse.data.places) {
        ...
        const sortedPlaces = mapsResponse.data.places
