function sendQuery() {
  ...
  const useStatic =
    new URLSearchParams(window.location.search).get('useStatic') === 'true';
  ...
  fetch(url, {
    ...
    .then((data) => {
      ...
      if (data.places && data.places.length) {
        // Adjusted for the actual data structure
        if (data.places && data.places.length) {
          // Adjusted for the actual data structure
          data.places.slice(0, 5).forEach((result, index) => {
            // Limit iteration to the first five results
            setTimeout(() => {
          ...
          resultsContainer.innerHTML =
            '<p>No results found or invalid query. <u><a href="/support">Help me out! 😰</a></u></p>';
          button.innerHTML = 'Search';
          button.disabled = false;
        }
      } else {
    ...
    .catch((error) => {
      ...
      document.getElementById('results').innerHTML =
        '<p>An error occurred fetching the results.</p>';
      button.innerHTML = 'Search';
      button.disabled = false;
    });
}
...
function evenMoreResults() {
  ...
  fetch(url, {
    ...
    .then((data) => {
      ...
      if (data.places && data.places.length) {
        // Adjusted for the actual data structure
        data.places.slice(5, 30).forEach((result, index) => {
          // Limit iteration to the first five results
          setTimeout(() => {
            const resultItem = document.createElement('a');
            resultItem.className = 'result-item';
