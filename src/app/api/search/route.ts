...
const aiRequest = async (query, country, retryQuery = null) => {
  ...
  const queryPrefix = fs.readFileSync(path.join(__dirname, queryFile), 'utf8').
    trim()
...
const mapsRequestWithDistance = async (
...
export async function POST (req: Request) {
    ...
    if (searchParams.get('recentSearches') === 'true') {
      try {
        ...
        return res.status(500).
          json({ error: 'Failed to fetch recent searches' })
      }
    }
  ...
  async function handleRequest (
    ...
    try {
      ...
      return res.status(500).
        json({ error: error.message || 'Unknown error', searchId: searchId })
    }
  }
