...
export async function getCachedResults(query) {
  try {
    ...
    const cachedResults = await resultsCollection
      .find({
        $or: [
...
export const aiRequest = async (query, country, retryQuery = null) => {
  ...
  //   trim()
  const fullAiContent =
    retryQuery || `${queryPrefix} ${country ? `${country} ` : ""} ${query}`
  ...
  try {
    ...
    if (
      ...
      aiResponse.data.candidates.length > 0 &&
      ...
      try {
        ...
        if (
          !parsedResponse.aiQuery ||
          typeof parsedResponse.aiQuery !== "string"
        ) {
          throw new Error(
...
export const mapsRequestWithDistance = async (
  ...
  if (latitude && longitude && mapsResponse.places) {
    mapsResponse.places = addDistanceToPlaces(
      mapsResponse.places,
      latitude,
      longitude,
    )
...
export async function getRecentSearches(): Promise<QueryInfo> {
  try {
    ...
    const recentSearches = await resultsCollection
      .find({
    ...
    console.log(
      "Formatted unique recent searches:",
  ...
  async function handleRequest(
    ...
    try {
      const aiResult = await aiRequest(
        query,
        isLatLongMode ? `modeisLatLong:${country || ""}` : country,
        retryQuery,
      )
      ...
      const mapsResponse = await mapsRequestWithDistance(
        aiResponse.aiQuery,
        latitude,
        longitude,
        radius,
      ...
      const sortedPlaces = await processor(
        mapsResponse,
        aiResponse.aiQuery,
        hasLocationInfo,
        isLatLongMode,
      ...
      if ((retryCondition1 || retryCondition2) && retry) {
        ...
        const newRetryQuery = `${aiContent} 5. IMPORTANT Your previous response was (${JSON.stringify(
          aiResponse,
        )}) which gave no results in Google Maps API. Please provide a different query, focusing on the type of place and its characteristics, without mentioning specific locations.`
